...
export function DividerNode({ id, data }: NodeProps<DividerNodeData>) {
  ...
  return (
    <div
      ref={ref}
      className="relative min-w-[138px] rounded-[2px] border border-[#4aa3ff] bg-white px-2 py-1 text-[9px] text-[#666666] shadow-none"
    >
      ...
      <div className="mt-1 mr-5 relative">
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          className="flex w-full items-center justify-between rounded-[2px] border border-[#c9ced6] bg-white px-1 py-[2px] text-[9px] text-[#666666] outline-none disabled:cursor-not-allowed disabled:opacity-60"
        >
          <span className="text-[#666666]">÷ {String(currentValue)}</span>
          <ChevronDown size={10} className="text-[#9aa3af]" />
        </button>

        {open ? (
          <div className="absolute left-0 top-full z-50 mt-1 w-full rounded-[2px] border border-[#c9ced6] bg-white shadow-lg">
            {options.map((opt) => {
