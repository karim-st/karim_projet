...
function FlowCanvasContent({ nodes, edges }: FlowCanvasProps) {
  ...
  const renderedNodes = useMemo(
    () => flowNodes.map((node) => {
      ...
      const active = highlighted?.nodeIds.has(node.id) ?? true;
      return {
        ...node,
        ...
        style: {
          ...node.style,
          outline: isSelected && !isGroup ? "2px solid #087fdb" : undefined,
          outlineOffset: isSelected && !isGroup ? "2px" : undefined,
          opacity: highlighted ? (active ? 1 : 0.28) : 1,
          filter: highlighted
            ? active
    ...
    [flowNodes, highlighted, isReadOnly, selectedNodeIds]
  );
  const fanOutGroups = useMemo(() => {
  ...
  const renderedEdges = useMemo(
    () =>
      edges.map((edge, index) => {
        ...
        const fanOutGroup = fanOutGroups.get(sourceKey) ?? [];
        const isFanOut = fanOutGroup.length > 1;
        const fanOutIndex = fanOutGroup.indexOf(edge.id);
        const branchOffset = isFanOut ? (fanOutIndex - (fanOutGroup.length - 1) / 2) * 18 : 0;
        
        return {
        ...edge,
        selected: edge.id === selectedEdgeId,
        zIndex: edge.id === selectedEdgeId ? 20 : isHighlighted ? 10 : 0,
        label: String(edge.targetHandle ?? "").includes("__") ? undefined : edge.label,
        type: edge.type ?? "step",
        style: hasError
        ...
        markerEnd: String(edge.targetHandle ?? "").includes("__")
          ? undefined
          : { ...marker(), color: hasError ? "#ef4444" : darkMode ? "#ffffff" : edge.id === selectedEdgeId || isHighlighted ? "#087fdb" : "#7b8798" },
        data: {
          ...(edge as any).data,
          ...
          fanOut: isFanOut ? { branchOffset, showJunction: fanOutIndex === 0 } : undefined
        }
      }; }),
    [darkMode, edges, highlighted, isReadOnly, selectedEdgeId, fanOutGroups]
  );
    ...
    >
      ...
      >
            ...
            {([
              ...
              return (
                <button
                  ...
                  className={`flex h-8 w-8 items-center justify-center border-r border-gray-200 last:border-r-0 disabled:cursor-not-allowed disabled:opacity-40 ${
        ...
        <div className={`absolute left-3 top-3 z-20 flex items-center overflow-hidden rounded-md border shadow-md ${darkMode ? "border-slate-500 bg-[#202b38]" : "border-gray-300 bg-white"}`} role="toolbar" aria-label="Annotations du schéma">
          <button type="button" disabled={isReadOnly} title="Ajouter un texte" aria-label="Ajouter un texte" onClick={() => addAnnotation("annotationText")} className="flex h-8 w-8 items-center justify-center border-r border-gray-200 text-gray-600 hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-40">
            <Type size={15} aria-hidden="true" />
          </button>
          <input disabled={isReadOnly} type="number" min="8" max="72" value={annotationFontSize} onChange={(event) => setAnnotationTextSize(Number(event.target.value))} className="h-8 w-12 border-r border-gray-200 bg-white px-1 text-[10px] text-gray-700 outline-none disabled:cursor-not-allowed disabled:opacity-40" aria-label="Taille du texte" title="Taille du texte" />
          <button type="button" disabled={isReadOnly} title="Ajouter un rectangle" aria-label="Ajouter un rectangle" onClick={() => addAnnotation("annotationRectangle")} className="flex h-8 w-8 items-center justify-center border-r border-gray-200 text-gray-600 hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-40">
            <RectangleHorizontal size={16} aria-hidden="true" />
          </button>
          <button type="button" disabled={isReadOnly} title="Ajouter un carré" aria-label="Ajouter un carré" onClick={() => addAnnotation("annotationSquare")} className="flex h-8 w-8 items-center justify-center border-r border-gray-200 text-gray-600 hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-40">
            <Square size={15} aria-hidden="true" />
          </button>
          <button type="button" disabled={isReadOnly} title="Ajouter un cercle" aria-label="Ajouter un cercle" onClick={() => addAnnotation("annotationCircle")} className="flex h-8 w-8 items-center justify-center text-gray-600 hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-40">
          ...
          {backgroundOptions.map((option) => {
            ...
            return (
              <button
                ...
                className={`flex h-8 w-8 items-center justify-center border-r border-gray-200 last:border-r-0 ${isActive ? "bg-[#17146E] text-white" : "text-gray-600 hover:bg-gray-100"}`}
              >
      ...
      {contextMenu && (
        <div
          className="fixed z-50 w-44 overflow-hidden rounded-md border border-gray-200 bg-white py-1 text-xs text-gray-700 shadow-lg"
        ...
        >
          {!contextMenu.nodeId && (
            <button
              ...
              className="flex w-full items-center gap-2 px-3 py-2 text-left hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-40"
              onClick={() => {
                pasteClipboard();
                setContextMenu(null);
              }}
