...
export function ClockBlockNode({ id, data }: NodeProps<ClockData>) {
  ...
  return (
    <div className="relative min-w-[150px] rounded-[2px] border border-[#c9ced6] bg-white px-2 py-1 text-[9px] text-[#666666] shadow-none dark:border-[#2f3e52] dark:bg-[#1e293b] dark:text-slate-300">
      <div className="text-[10px] font-medium leading-none text-[#666666] dark:text-slate-300">
        {title}
      </div>

      <div className="mt-1 mr-5 flex items-center gap-1 rounded-[2px] border border-[#c9ced6] bg-white px-1 py-[2px] dark:border-[#2f3e52] dark:bg-[#243349]">
        <select
          value={unitText}
          disabled={data?.isLocked}
          onChange={(e) => handleUnitChange(e.target.value)}
          className="min-w-[52px] bg-transparent text-[9px] text-[#9aa3af] outline-none disabled:cursor-not-allowed dark:text-slate-400"
        >
