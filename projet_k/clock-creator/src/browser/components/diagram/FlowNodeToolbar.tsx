  ...
  return (
    <div className="mb-2 flex items-start gap-2">
        ...
        >
          <button
            ...
            className={`flex h-9 min-w-48 items-center justify-between gap-3 rounded-md border px-3 text-xs font-medium shadow-sm transition-colors focus:outline-none focus:ring-2 focus:ring-[#17146E]/25 ${
              isTypeMenuOpen
                ? "border-[#17146E] bg-[#17146E]/5 text-[#17146E] dark:border-[#8b89f5] dark:bg-[#17146E]/40 dark:text-[#b3b1ff]"
                : "border-gray-300 bg-white text-gray-700 hover:border-[#17146E]/50 hover:bg-gray-50 dark:border-slate-600 dark:bg-[#1e293b] dark:text-slate-200 dark:hover:bg-[#27354a]"
            }`}
            disabled={isReadOnly}
            aria-haspopup="menu"
          ...
          {isTypeMenuOpen && (
            <div className="absolute left-0 top-full z-20 mt-1 w-48 rounded-md border border-gray-200 bg-white py-1 text-xs shadow-lg dark:border-slate-600 dark:bg-[#1e293b] dark:text-slate-200" role="menu">
              {CLOCK_TYPES.map((type) => (
                <div
                  key={type.value}
                  className="relative"
                ...
                >
                  <button
                    type="button"
                    disabled={isReadOnly}
                    className={`flex w-full items-center px-2 py-1.5 text-left hover:bg-gray-100 dark:hover:bg-slate-700 ${selectedType === type.value ? "bg-gray-100 dark:bg-slate-700" : ""}`}
                    role="menuitem"
                    onClick={() => {
                  ...
                  {type.value === "multiplexer" && isMuxSubmenuOpen && (
                    <div className="absolute left-full top-0 w-52 border border-gray-300 bg-white py-1 shadow-md dark:border-slate-600 dark:bg-[#1e293b] dark:text-slate-200" role="menu" aria-label="Multiplexer profiles">
                      {Object.entries(MUX_PROFILES).map(([profile, definition]) => (
                        <button
                          key={profile}
                          type="button"
                          disabled={isReadOnly}
                          className={`w-full px-2 py-1.5 text-left hover:bg-gray-100 dark:hover:bg-slate-700 ${selectedMuxProfile === profile ? "bg-gray-100 dark:bg-slate-700" : ""}`}
                          role="menuitem"
                          onClick={() => {
      ...
      <div className="relative min-w-52">
        <Search size={14} aria-hidden="true" className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 dark:text-slate-400" />
        <input
          type="search"
          disabled={isReadOnly}
          value={searchQuery}
          ...
          className="h-9 w-full rounded-md border border-gray-300 bg-white pl-8 pr-2 text-xs text-gray-700 outline-none transition focus:border-[#17146E] focus:ring-2 focus:ring-[#17146E]/20 disabled:cursor-not-allowed disabled:opacity-50 dark:border-slate-600 dark:bg-[#0f1a2a] dark:text-slate-100"
        />
        {isSearchOpen && searchQuery.trim() && (
          <div className="absolute left-0 top-full z-30 mt-1 max-h-56 w-full overflow-y-auto rounded-md border border-gray-200 bg-white py-1 text-xs shadow-lg dark:border-slate-600 dark:bg-[#1e293b] dark:text-slate-200" role="listbox" aria-label="Résultats de composants">
            {searchResults.length > 0 ? searchResults.map((component) => (
              <button
                key={component.id}
                type="button"
                role="option"
                className="flex w-full flex-col px-3 py-2 text-left text-gray-700 hover:bg-[#e8f4fb] dark:text-slate-200 dark:hover:bg-slate-700"
                onMouseDown={(event) => event.preventDefault()}
                onClick={() => addSearchResult(component)}
              >
                <span>{component.label}</span>
                <span className="mt-0.5 text-[10px] text-gray-500 dark:text-slate-400">{component.detail}</span>
              </button>
            )) : (
              <div className="px-3 py-2 text-gray-500 dark:text-slate-400">Aucun composant</div>
            )}
          </div>
        )}
      </div>
      ...
      <Button
        ...
        className={`ml-auto ${isReadOnly ? "border-red-400 bg-red-50 text-red-700 dark:border-red-900 dark:bg-red-950/40 dark:text-red-400" : ""}`}
      >
